...
const tabs = [
  ...
  "Fixed Assets", "Free Cash Flow", "Over-Performance", "TOCC", "Debt Schedule", "Liquidity", "Interest Tax Shield",
...
const op = model.overperformance || [];
const s = model.summary;
const a = model.assumptions;
const company = model.company;
const forecastEnd = colName(f.length + 1);
const overperformanceEnd = colName(op.length + 1);
...
// Cover
{
  ...
  sh.getRange("D5:H10").merge(true); sh.getRange("D5:D10").values = [["Operating value is discounted at unlevered TOCC."], ["Explicit, competitive-advantage fade, and steady-state periods are valued separately."], ["Terminal RONIC equals TOCC; no excess return persists forever."], ["Tax shields use parallel NOL schedules with and without interest."], ["Liquidity shortfalls are disclosed rather than silently funded."], ["Review Sources, Value Drivers, Scenarios, and Model Checks before use."]];
...
// Assumptions
{
  ...
  const rows = [["Explicit forecast years", a.forecast_years, "years", "Detailed operating period"], ["Competitive-advantage years", a.competitive_advantage_years, "years", "RONIC fades to TOCC"], ["Starting revenue growth", a.revenue_growth_start, "%", "Historical calibration"], ["Terminal revenue growth", a.revenue_growth_terminal, "%", "Normalized long-run growth"], ["Starting EBIT margin", a.ebit_margin_start, "%", "Historical calibration"], ["Terminal EBIT margin", a.ebit_margin_terminal, "%", "Sustainable operating hypothesis"], ["Normalized tax rate", a.tax_rate, "%", "Operating cash-tax rate"], ["Initial operating NOL", a.initial_operating_nol, "$mm", "Parallel tax schedules"], ["Risk-free rate", a.risk_free_rate, "%", "Currency-matched"], ["Market risk premium", a.market_risk_premium, "%", "Implied market premium"], ["Selected asset beta", a.selected_asset_beta, "beta", "Operating-risk beta"], ["TOCC", s.tocc, "%", "Risk-free + beta × MRP"], ["Terminal growth", a.terminal_growth_rate, "%", "Must be below TOCC"], ["Terminal RONIC", s.terminal_ronic, "%", "Locked to TOCC in true steady state"], ["Minimum cash", a.minimum_cash, "$mm", "Liquidity threshold"], ["Interest limitation", a.interest_limit_percentage, "% ATI", "Section 163(j)-style convention"], ["Annual SBC dilution", a.annual_sbc_dilution_rate, "%", "Scenario share-count schedule"]];
...
// Competitive-advantage fade
{
  const sh = workbook.worksheets.getItem("Over-Performance"); title(sh, "Competitive-Advantage Fade", "RONIC converges to TOCC before true steady state", overperformanceEnd);
  sh.getRange(`A4:${overperformanceEnd}4`).values = [["Metric", ...op.map(r => r.year)]]; head(sh.getRange(`A4:${overperformanceEnd}4`));
  const metrics = [["Revenue growth", "revenue_growth", pct], ["Revenue", "revenue", money], ["EBIT margin", "ebit_margin", pct], ["NOPAT", "nopat", money], ["Net new investment", "net_investment", money], ["RONIC", "ronic", pct], ["ROIC", "roic", pct], ["Unlevered FCF", "fcf", money], ["Ending cash", "ending_cash", money]];
  if (op.length) { sh.getRange(`A5:${overperformanceEnd}${4 + metrics.length}`).values = metrics.map(([label, key]) => [label, ...op.map(r => r[key])]); body(sh.getRange(`A5:${overperformanceEnd}${4 + metrics.length}`)); metrics.forEach((m, i) => sh.getRange(`B${5 + i}:${overperformanceEnd}${5 + i}`).format.numberFormat = m[2]); sh.getRange(`B5:${overperformanceEnd}${4 + metrics.length}`).format.fill = "#E4DFEC"; }
  sh.getRange("A:A").format.columnWidth = 32; sh.getRange(`B:${overperformanceEnd}`).format.columnWidth = 14; sh.freezePanes.freezeRows(4); sh.freezePanes.freezeColumns(1);
}
...
// TOCC
{
  const sh = workbook.worksheets.getItem("TOCC"); title(sh, "The Opportunity Cost of Capital", "Peer asset-beta audit and selected operating-risk hurdle", "I");
...
// Liquidity
{
  const sh = workbook.worksheets.getItem("Liquidity"); title(sh, "Liquidity and Financing Capacity", "Cash roll-forward; financing needs are identified, not plugged", "I");
...
// Tax shield
{
  const sh = workbook.worksheets.getItem("Interest Tax Shield"); title(sh, "Interest Deductibility, NOLs, and Tax Shield", "Parallel cash-tax schedules isolate the incremental financing benefit", "R"); sh.getRange("A4:R4").values = [["Year", "Phase", "Interest", "ATI", "30% ATI limit", "Opening interest CF", "CF used", "Deductible interest", "Ending interest CF", "Opening NOL w/o interest", "NOL used w/o interest", "Ending NOL w/o interest", "Tax w/o interest", "Opening NOL with interest", "NOL used with interest", "Ending NOL with interest", "Tax with interest", "Usable tax shield"]]; const rows = model.tax_shield.map(r => [r.year, r.phase, r.interest, r.ati, r.limit, r.opening_carryforward, r.carryforward_used, r.deductible_interest, r.ending_carryforward, r.opening_nol_without_interest, r.nol_used_without_interest, r.ending_nol_without_interest, r.cash_tax_without_interest, r.opening_nol_with_interest, r.nol_used_with_interest, r.ending_nol_with_interest, r.cash_tax_with_interest, r.usable_tax_shield]); sh.getRange(`A5:R${4 + rows.length}`).values = rows; setupTable(sh, `A5:R${4 + rows.length}`, "A4:R4"); sh.getRange(`C5:R${4 + rows.length}`).format.numberFormat = money; sh.getRange("A:A").format.columnWidth = 12; sh.getRange("B:B").format.columnWidth = 20; sh.getRange("C:R").format.columnWidth = 18;
  const noteRow = 7 + rows.length; sh.getRange(`A${noteRow}:R${noteRow}`).merge(); sh.getRange(`A${noteRow}`).values = [["Usable shield equals cash tax without interest less cash tax with deductible interest, after NOL and Section 163(j)-style carryforwards."]]; sh.getRange(`A${noteRow}:R${noteRow}`).format = { fill: C.yellow, wrapText: true, rowHeight: 32 };
}

// Continuing value
{
  const sh = workbook.worksheets.getItem("Continuing Value"); title(sh, "Competitive Advantage and Continuing Value", "Explicit fade period followed by true steady state", "D"); sh.getRange("A4:B4").values = [["Input / calculation", "Value"]]; head(sh.getRange("A4:B4")); sh.getRange("A5:A15").values = [["Final fade-period NOPAT"], ["Terminal growth"], ["Terminal RONIC"], ["TOCC"], ["NOPAT N+1"], ["Reinvestment rate"], ["Terminal FCF"], ["Continuing value at fade-period end"], ["PV over-performance FCF"], ["PV terminal value"], ["PV continuing value"]]; sh.getRange("B5:B8").values = [[op.length ? op[op.length - 1].nopat : f[f.length - 1].nopat], [s.terminal_growth], [s.terminal_ronic], [s.tocc]]; sh.getRange("B9").formulas = [["=B5*(1+B6)"]]; sh.getRange("B10").formulas = [["=B6/B7"]]; sh.getRange("B11").formulas = [["=B9*(1-B10)"]]; sh.getRange("B12").formulas = [["=IF(OR(B6>=B8,B7<=0,B10<0,B10>1),\"\",B11/(B8-B6))"]]; sh.getRange("B13:B15").values = [[s.pv_overperformance_fcf], [s.pv_terminal_value], [s.pv_continuing_value]]; body(sh.getRange("A5:B15")); sh.getRange("B5").format.numberFormat = money; sh.getRange("B6:B10").format.numberFormat = pct; sh.getRange("B11:B15").format.numberFormat = money; sh.getRange("B6:B8").format = { fill: C.yellow, font: { color: C.input } }; sh.getRange("A:A").format.columnWidth = 42; sh.getRange("B:B").format.columnWidth = 22; sh.getRange("D5:D10").values = [["Terminal g < TOCC"], ["Terminal RONIC = TOCC"], ["Reinvestment 0%–100%"], ["RONIC fade completed"], ["No perpetual excess return"], ["All terminal cash flows discounted from fade-period end"]]; sh.getRange("D:D").format.columnWidth = 42;
}

// APV
{
  const sh = workbook.worksheets.getItem("APV"); title(sh, "Adjusted Present Value", "Operating value plus separately valued financing effects", "D"); sh.getRange("A4:B4").values = [["Adjusted Present Value and Equity Bridge", "Value"]]; head(sh.getRange("A4:B4")); sh.getRange("A5:A13").values = [["PV of explicit unlevered FCF"], ["PV of continuing value"], ["Operating enterprise value"], ["PV of explicit interest tax shields"], ["PV of continuing interest tax shield"], ["PV of other financing effects"], ["PV of financing effects"], ["APV enterprise value"], ["Continuing value / APV enterprise value"]]; sh.getRange("B5:B6").values = [[s.pv_explicit_fcf], [s.pv_continuing_value]]; sh.getRange("B7").formulas = [["=SUM(B5:B6)"]]; sh.getRange("B8:B10").values = [[s.pv_explicit_tax_shields], [s.pv_continuing_tax_shield], [s.pv_other_financing_effects]]; sh.getRange("B11").formulas = [["=SUM(B8:B10)"]]; sh.getRange("B12").formulas = [["=B7+B11"]]; sh.getRange("B13").formulas = [["=IF(B12=0,\"\",B6/B12)"]]; body(sh.getRange("A5:B13")); sh.getRange("B5:B12").format.numberFormat = money; sh.getRange("B13").format.numberFormat = pct; sh.getRange("B5:B13").format.font = { color: C.linked, bold: true }; sh.getRange("A:A").format.columnWidth = 44; sh.getRange("B:B").format.columnWidth = 22;
}

// Equity bridge
{
  const sh = workbook.worksheets.getItem("Equity Bridge"); title(sh, "Enterprise Value to Equity Value", "Only excess/non-operating cash is added; equity is floored at zero", "D"); sh.getRange("A4:B4").values = [["Equity bridge", "Value"]]; head(sh.getRange("A4:B4")); sh.getRange("A5:A14").values = [["APV enterprise value"], ["Less: gross debt"], ["Less: operating lease liabilities / other claims"], ["Less: pension, preferred, and minority claims"], ["Add: excess cash and marketable securities"], ["Add: non-operating investments"], ["Other non-operating adjustments"], ["Equity value (limited-liability floor)"], ["Diluted shares"], ["Intrinsic value per share"]]; sh.getRange("B5").formulas = [["='APV'!B12"]]; sh.getRange("B6:B11").values = [[s.gross_debt], [company.historical[company.historical.length - 1].lease_liabilities], [Math.max(0, s.other_financing_claims - company.historical[company.historical.length - 1].lease_liabilities)], [s.excess_cash], [company.non_operating_investments], [0]]; sh.getRange("B12").formulas = [["=MAX(0,B5-B6-B7-B8+B9+B10+B11)"]]; sh.getRange("B13").values = [[s.diluted_shares]]; sh.getRange("B14").formulas = [["=IF(B13=0,\"\",B12/B13)"]]; body(sh.getRange("A5:B14")); sh.getRange("B5:B12").format.numberFormat = money; sh.getRange("B13").format.numberFormat = count; sh.getRange("B14").format.numberFormat = perShare; sh.getRange("B5:B5").format.font = { color: C.linked }; sh.getRange("B12:B14").format.font = { bold: true, color: "#000000" }; sh.getRange("A:A").format.columnWidth = 52; sh.getRange("B:B").format.columnWidth = 22;
}

// Market comparison
{
  const sh = workbook.worksheets.getItem("Market Comparison"); title(sh, "Intrinsic Value versus Market Price", "Market price is not an input to intrinsic operating value", "D"); sh.getRange("A4:B4").values = [["Metric", "Value"]]; head(sh.getRange("A4:B4")); sh.getRange("A5:A11").values = [["Intrinsic value per share"], ["Market price"], ["Market capitalization"], ["Premium / (discount) to market"], ["Market-implied equity value"], ["Intrinsic equity value"], ["Dollar difference per share"]]; sh.getRange("B5").formulas = [["='Equity Bridge'!B14"]]; sh.getRange("B6:B7").values = [[s.market_price], [s.market_cap]]; sh.getRange("B8").formulas = [["=IF(B6=0,\"\",B5/B6-1)"]]; sh.getRange("B9").formulas = [["=B6*'Equity Bridge'!B13"]]; sh.getRange("B10").formulas = [["='Equity Bridge'!B12"]]; sh.getRange("B11").formulas = [["=B5-B6"]]; body(sh.getRange("A5:B11")); sh.getRange("B5:B6").format.numberFormat = perShare; sh.getRange("B7").format.numberFormat = money; sh.getRange("B8").format.numberFormat = pct; sh.getRange("B9:B10").format.numberFormat = money; sh.getRange("B11").format.numberFormat = perShare; sh.getRange("B5:B5").format.font = { color: C.linked }; sh.getRange("A:A").format.columnWidth = 42; sh.getRange("B:B").format.columnWidth = 22;
}

// Scenarios
{
  const sh = workbook.worksheets.getItem("Scenarios"); title(sh, "Story-Driven Valuation Scenarios", "Operating outcomes, financing, dilution, liquidity, and liquidation", "J"); sh.getRange("A4:J4").values = [["Scenario", "Probability", "Type", "TOCC", "Terminal growth", "APV EV", "Equity value", "Diluted shares", "Intrinsic value / share", "Probability-weighted value"]]; const rows = model.scenarios.map(r => [r.scenario, r.probability, r.liquidation ? "Liquidation" : "Going concern", r.tocc, r.terminal_growth, r.apv_enterprise_value, r.equity_value, r.diluted_shares, r.intrinsic_value_per_share, r.probability * r.intrinsic_value_per_share]); sh.getRange(`A5:J${4 + rows.length}`).values = rows; setupTable(sh, `A5:J${4 + rows.length}`, "A4:J4"); sh.getRange(`B5:B${4 + rows.length}`).format.numberFormat = pct; sh.getRange(`D5:E${4 + rows.length}`).format.numberFormat = pct; sh.getRange(`F5:G${4 + rows.length}`).format.numberFormat = money; sh.getRange(`H5:H${4 + rows.length}`).format.numberFormat = count; sh.getRange(`I5:J${4 + rows.length}`).format.numberFormat = perShare; const total = 6 + rows.length; sh.getRange(`A${total}:B${total}`).values = [["Probability total", null]]; sh.getRange(`B${total}`).formulas = [[`=SUM(B5:B${4 + rows.length})`]]; sh.getRange(`H${total}:I${total}`).values = [["Expected intrinsic value", null]]; sh.getRange(`I${total}`).formulas = [[`=SUM(J5:J${4 + rows.length})`]]; sh.getRange(`B${total}`).format.numberFormat = pct; sh.getRange(`I${total}`).format.numberFormat = perShare; sh.getRange("A:J").format.columnWidth = 19;
}
...
// Dashboard
{
  ...
  sh.getRange("A4:D4").values = [["KPI", "Value", "KPI", "Value"]]; head(sh.getRange("A4:D4")); sh.getRange("A5:D10").values = [["Intrinsic value / share", s.intrinsic_value_per_share, "Market price", s.market_price], ["Premium / (discount)", s.premium_discount, "TOCC", s.tocc], ["APV enterprise value", s.apv_enterprise_value, "Equity value", s.equity_value], ["PV continuing value / APV", s.continuing_value_share, "Terminal RONIC", s.terminal_ronic], ["Overall status", s.overall_model_status, "Forecast years", f.length], ["Market capitalization", s.market_cap, "Historical years", h.length]]; body(sh.getRange("A5:D10")); sh.getRange("B5:D10").format.font = { bold: true }; sh.getRange("B5:D5").format.numberFormat = perShare; sh.getRange("B6:D6").format.numberFormat = pct; sh.getRange("B7:D7").format.numberFormat = money; sh.getRange("B8:D8").format.numberFormat = pct; sh.getRange("B10").format.numberFormat = money; sh.getRange("D10").format.numberFormat = count; statusFormatting(sh.getRange("B9"));
  const combined = [...h.map(r => ({ year: r.year, revenue: r.revenue, margin: r.ebit_margin, roic: r.roic, tocc: r.tocc, fcf: r.fcf })), ...f.map(r => ({ year: r.year, revenue: r.revenue, margin: r.ebit_margin, roic: r.roic, tocc: s.tocc, fcf: r.fcf })), ...op.map(r => ({ year: r.year, revenue: r.revenue, margin: r.ebit_margin, roic: r.roic, tocc: s.tocc, fcf: r.fcf }))];
